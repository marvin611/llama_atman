...
class LlamaSdpaAttention(LlamaAttention, LlamaAttentionManipulationMixin):
    """
    ...
    def forward(
        ...
        modified_causal_attention_mask: Optional[torch.Tensor] = None,
        manipulate_attn_scores_after_scaling: bool = False,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        if output_attentions:
            # Fall back to eager implementation for output_attentions
        ...
        # Create base causal mask if none exists
        ...
        # Apply manipulation before scaling if specified
        if not manipulate_attn_scores_after_scaling and modified_causal_attention_mask is not None:
            causal_mask = self.manipulate_attention_weights(
                causal_mask, 
                attention_mask,
                modified_causal_attention_mask
            )

        # Use SDPA with the manipulated mask
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            ...
            is_causal=True if causal_mask is None and query_states.size(-2) > 1 else False,
        )

        # Apply manipulation after scaling if needed
        if manipulate_attn_scores_after_scaling and modified_causal_attention_mask is not None:
            attn_output = self.manipulate_attention_weights(
                attn_output,
                attention_mask, 
                modified_causal_attention_mask
            )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if manipulate_layers is not None and len(manipulate_layers) > 0:
                if idx not in manipulate_layers:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # Create modified attention mask if suppression is active
        ...
        if self.suppression_token_indices is not None and self.suppression_factors is not None:
            if input_ids is not None:
                seq_len = input_ids.shape[1] if past_key_values is None else past_key_values[-1][0].shape[2]
            else:
    ...
    def get_modified_causal_attention_mask(
        self, 
        seq_len: int,
        suppression_token_indices: List[List[int]], 
        suppression_factors: List[List[float]],
        device: torch.device,
        dtype: torch.dtype,
        batch_size: int,
    ) -> torch.Tensor:
        ...
        for batch_idx in range(batch_size):
            assert len(suppression_token_indices[batch_idx]) == len(suppression_factors[batch_idx]), \
                f"Mismatch in suppression specs for batch {batch_idx}"
                
            for token_idx, factor in zip(
                suppression_token_indices[batch_idx],
                suppression_factors[batch_idx]
            ):
                if token_idx == -1 or token_idx is None:
                    continue
                
                attention_mask_factors[batch_idx, :, :, token_idx] = factor
