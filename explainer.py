...
class Explainer:
    def __init__(self, model, device, tokenizer, suppression_factor = 0.1, conceptual_suppression_threshold = 0.6,
        ...
        self.model.model.suppression_factor = suppression_factor
        ...
        self.model.model.conceptual_suppression_threshold = conceptual_suppression_threshold
    ...
    def collect_logits_by_manipulating_attention(self, prompt: list, target: str, prompt_explain_indices: list = None, max_batch_size: int = 1, configs: dict = None, save_configs_as: str = None, save_configs_only: bool = False):
        ...
        target_token_ids = self.model.preprocess_inputs(target, embed=False)[0]
        if isinstance(target_token_ids, int):
            target_token_ids = [target_token_ids]  # Convert single int to list
        ...
        # Use target embeddings shape for sequence length calculation
        target_seq_len = target_embeddings.shape[1]
        
        # total_seq_len should use the actual target sequence length
        total_seq_len = prompt_seq_len + target_seq_len

        # target_token_indices should also use actual sequence length
        target_token_indices = list(
            range(prompt_length, prompt_length + target_seq_len - 1)
        )
    ...
    def collect_logits(self, batches: list, input_embeddings):
        ...
        for batch in batches:
            ...
            self.model.model.suppression_factors = suppression_factors
            self.model.model.suppression_token_indices = suppression_token_indices
            self.model.model.manipulate_attn_scores_after_scaling = self.manipulate_attn_scores_after_scaling
            ...
            self.model.model.suppression_token_indices = None
            self.model.model.suppression_factors = None
